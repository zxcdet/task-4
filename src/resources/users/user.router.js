...
router
  ...
  .post(validateMiddleware(userSchema), wrapAsync(async (req, res) => {
...
router
  ...
  .put(validateMiddleware(paramSchema, 'params'),validateMiddleware(userSchema),wrapAsync(async (req, res) => {
      const id = req.params.id;
